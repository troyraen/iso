...
def load_ISO(dc, ga) -> '[ dc, isochrones ]':
    ...
    dc.append(load_data('isochrones.csv'))
    isochrones = dc[0]
    # dc.append(load_data('profilesDF.csv'))
    # profiles = dc[1]
    # dc.append(load_data('descDF.csv'))
    # desc = dc[2]
    globals()['dclist'] = [ dc, isochrones ]
    return dclist
gfncs['load_ISO']= {'name':'load_ISO', \
                    'Signature':inspect.signature(load_ISO), \
                    'Docstring':inspect.getdoc(load_ISO), \
                    'class':'data'}
